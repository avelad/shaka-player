...
shaka.ads.InterstitialAdManager = class {
      ...
      if (this.interstitials_.size && this.lastTime_ != null) {
        ...
        for (const interstitial of interstitials) {
          ...
          if ((seekRange.start - comparisonTime) >= 1) {
            ...
            this.removePreloadImages_(interstitial);
            ...
            if (difference > 0 && difference <= 10) {
              ...
              this.checkPreloadImages_(interstitial);
            }
          }
        }
        if (cuepointsChanged) {
  ...
  stop() {
    ...
    if (this.preloadImages_.size) {
      const interstitials = Array.from(this.preloadImages_.keys());
      for (const interstitial of interstitials) {
        this.removePreloadImages_(interstitial);
      }
    }
  ...
  async addMetadata(hlsInterstitial) {
    ...
    const adInterstitials = overlayId ?
        this.getOverlayInfo_(hlsInterstitial) :
        await this.getInterstitialsInfo_(hlsInterstitial);
   ...
   * @private
   */
  getOverlayInfo_(hlsInterstitial) {
    const interstitialsAd = [];
    if (!hlsInterstitial) {
      return interstitialsAd;
    }
    ...
    const assetMimeType =
        hlsInterstitial.values.find((v) => v.key == 'X-ASSET-MIMETYPE');
    let mimeType = null;
    if (assetMimeType) {
      mimeType = assetMimeType.data;
    }
    const depth = hlsInterstitial.values.find((v) => v.key == 'X-DEPTH');
    let displayOnBackground = false;
    if (depth) {
      if (depth.data == '0') {
        return interstitialsAd;
      }
      displayOnBackground = depth.data == '-1';
    }
    const xLoop = hlsInterstitial.values.find((v) => v.key == 'X-LOOP');
    let loop = false;
    if (xLoop) {
      loop = xLoop.data == 'YES';
    }
    const xBackground =
        hlsInterstitial.values.find((v) => v.key == 'X-BACKGROUND');
    let background = null;
    if (xBackground) {
      background = xBackground.data;
    }
    /** @type {!shaka.extern.AdPositionInfo} */
    const overlay = {
      viewport: {
    ...
    const viewport =
        hlsInterstitial.values.find((v) => v.key == 'X-VIEWPORT');
    if (viewport) {
      const data = /** @type {string} */(viewport.data);
      const viewportComponents = data.split('x');
      if (viewportComponents.length != 2) {
        return interstitialsAd;
      }
      overlay.viewport.x = parseFloat(viewportComponents[0]);
      overlay.viewport.y = parseFloat(viewportComponents[1]);
    }
    const overlayPosition =
        hlsInterstitial.values.find((v) => v.key == 'X-OVERLAY-POSITION');
    if (overlayPosition) {
      const data = /** @type {string} */(overlayPosition.data);
      const overlayPositionComponents = data.split('x');
      if (overlayPositionComponents.length != 2) {
        return interstitialsAd;
      }
      overlay.topLeft.x = parseFloat(overlayPositionComponents[0]);
      overlay.topLeft.y = parseFloat(overlayPositionComponents[1]);
    }
    const overlaySize =
        hlsInterstitial.values.find((v) => v.key == 'X-OVERLAY-SIZE');
    if (overlaySize) {
      const data = /** @type {string} */(overlaySize.data);
      const overlaySizeComponents = data.split('x');
      if (overlaySizeComponents.length != 2) {
        return interstitialsAd;
      }
      overlay.size.x = parseFloat(overlaySizeComponents[0]);
      overlay.size.y = parseFloat(overlaySizeComponents[1]);
    }
    let currentVideo = null;
    const squeezeCurrent = hlsInterstitial.values.find(
        (v) => v.key == 'X-SQUEEZECURRENT');
    if (squeezeCurrent) {
      const percentage = parseFloat(squeezeCurrent.data);
      if (percentage) {
        currentVideo = {
          viewport: {
            x: 1920,
        ...
        if (squeezeCurrentPosition) {
          const data = /** @type {string} */(squeezeCurrentPosition.data);
          const squeezeCurrentPositionComponents = data.split('x');
          if (squeezeCurrentPositionComponents.length != 2) {
            return interstitialsAd;
          }
          currentVideo.topLeft.x =
              parseFloat(squeezeCurrentPositionComponents[0]);
          currentVideo.topLeft.y =
              parseFloat(squeezeCurrentPositionComponents[1]);
        }
      }
    }
    interstitialsAd.push({
   ...
   * @private
   */
  checkPreloadImages_(interstitial) {
    if (this.preloadImages_.has(interstitial) ||
    ...
    this.preloadImages_.set(interstitial, links);
  }
   ...
   * @private
   */
  removePreloadImages_(interstitial) {
    if (!this.preloadImages_.has(interstitial)) {
      return;
    }
    const links = this.preloadImages_.get(interstitial);
    for (const link of links) {
      link.parentNode.removeChild(link);
    }
    this.preloadImages_.delete(interstitial);
  }
};
