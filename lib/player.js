...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  selectVariantTrack(track, clearBuffer = false, safeMargin = 0) {
    const selectMediaSourceMode = () => {
      ...
      const role = (variant.audio && variant.audio.roles &&
          variant.audio.roles[0]) || '';
      const audioLabel = variant.audio && variant.audio.label ?
          variant.audio.label : '';
      const hdrLevel = variant.video && variant.video.hdr ?
          variant.video.hdr : '';
      const spatialAudio = variant.audio && variant.audio.spatialAudio ?
          variant.audio.spatialAudio : false;
      const videoLayout = variant.video && variant.video.videoLayout ?
          variant.video.videoLayout : '';
      const channelCount = variant.audio && variant.audio.channelsCount ?
          variant.audio.channelsCount : 0;
      const audioCodec = variant.audio && variant.audio.codecs ?
